...
% here
R0 = 1*1e3*eye(identified_sys_dim + 1); % nominal:1e2 others:200,1000,2000 % [x; u]: sys_dim + inputdim(=1)
lambda = 0.9999; % nominal:0.9995 others:0.999,0.9999,1
...
for k = 1:steps
    ...
    % Tracking error
    error = r - (x(5)^2 + sin(x(6)));  % Error between reference and state
...
semilogy(abs((X_Aug(5,:).^2 + sin(X_Aug(6,:)))-r), 'b', 'LineWidth', 3)
set(gca, 'fontsize', 16);
...
nexttile(1)
plot(X_Aug(5,:).^2 + sin(X_Aug(6,:)), 'b', 'LineWidth', 3)
hold on
ref_sig = r*ones(1, size(X_Aug,2));
plot(ref_sig, 'k--', 'LineWidth', 2)
set(gca, 'fontsize', 16);
