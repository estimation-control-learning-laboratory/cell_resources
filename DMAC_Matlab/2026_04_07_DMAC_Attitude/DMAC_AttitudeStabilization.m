...
B_c = [zeros(3);
        inv(J)];
[plant.A, plant.B ] = VanLoanDiscretization(A_c, B_c, sim.dt)

plant.C     = [eye(3) zeros(3)];
plant.C_xi  = eye(6);
...
disp('Relative Error in A is')
disp(norm(Theta_kp1(:,1:plant.lx)-plant.A)/norm(plant.A))

disp('Relative Error in B is')
disp(norm(Theta_kp1(:,plant.lx+1:end)-plant.B)/norm(plant.B))
