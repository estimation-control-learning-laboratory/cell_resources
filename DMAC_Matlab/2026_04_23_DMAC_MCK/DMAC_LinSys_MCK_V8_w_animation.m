...
anim.update_every    = 1;     % update figure every step
...
for k = 1:sim.N
    ...
    % Optional direct update of next measured values
    y(:,k+1)  = plant.C * x(:,k+1);
...
function [hfig, h] = initialize_animation(sim, plant, log, r, Theta_k, K, x0)
    ...
    h.y   = plot(ax1, nan, nan, 'LineWidth', 1.5);
    h.ref = plot(ax1, nan, nan, '--', 'LineWidth', 1.2);
    xlabel(ax1,'Step');
    ylabel(ax1,'Output');
    ...
    h.u = plot(ax2, nan, nan, 'LineWidth', 1.5);
    xlabel(ax2,'Step');
    ylabel(ax2,'u_k');
    ...
    for i = 1:n_theta
        h.theta(i) = plot(ax3, nan, nan, 'LineWidth', 1.0);
    end
    xlabel(ax3,'Step');
    ...
    h.K = gobjects(n_K,1);
    for i = 1:n_K
        h.K(i) = plot(ax4, nan, nan, 'LineWidth', 1.0);
    end
    xlabel(ax4,'Step');
